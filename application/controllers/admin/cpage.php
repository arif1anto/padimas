...
class Cpage extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->database();
        $this->load->model(array('mpage','mkategori','mlogin'));
        $this->cek_sess();
    ...
    private function initpage($id=1){
        $key = $this->session->userdata('filter_page');
        $url ='admin/halaman/page';
        ...
        if(!empty($key)) {
            ...
            $this->session->unset_userdata('filter_page');
    ...
    public function cari(){
        if ($_POST) {
            ...
            $filter = $tgl.'&'.$key.'&'.$ket;
            $this->session->set_userdata('filter_page', $filter);
            $v = $this->initpage(1);
            $v['content'] = "admin/page";
            $this->global_view($v);
        } else {
