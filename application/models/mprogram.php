...
class Mprogram extends CI_Model {
	...
	function inprogram($form=null,$image=null,$jumbea=0){
		$data = array(
				'prg_nama' 		=> $this->input->post('nama_prg'),
				'prg_judul' 	=> $this->input->post('judul'),
				'prg_deskripsi' => $this->input->post('deskripsi',FALSE),
	...
	function upprogram($id=null, $form=null,$image=null,$jumbea=0) {
		$data = array(
				'prg_nama' 		=> $this->input->post('nama_prg'),
				'prg_judul' 	=> $this->input->post('judul'),
				'prg_deskripsi' => $this->input->post('deskripsi',FALSE),
		...
		//upload image
		if($image==null && $this->input->post('file_exist')=="1"){
			//no update image
		} else {
			$data = array('prg_image' => $image);
			$this->db->where('prg_id',$id);
			$this->db->update($this->tb,$data);
		}
		return true;
	...
	function getprogram_nonreg(){
		$q = $this->getprogram()
				->not_like("a.prg_nama","reguler")
			   ->order_by("a.prg_id","asc");
		return $q->get()->result();
	}
	
	function getprogram_reg(){
		$q = $this->getprogram()
				->like("a.prg_nama","reguler")
			   ->order_by("a.prg_id","asc");
		return $q->get()->result();
	}
